...
def sim_log2log(log1,delimiters1,brackets1,log2,delimiters2,brackets2):
    ...
    if(not delimiters1 and not delimiters2 and not brackets1 and not brackets2):
        if (re.match(r'\b(\-?\+?\d+)\b|\b0[Xx][a-fA-F\d]+\b|\b[a-fA-F\d]{4,}\b',log1) or log1=='<\d>' or log1=='-<\d>' ) and (re.match(r'\b(\-?\+?\d+)\b|\b0[Xx][a-fA-F\d]+\b|\b[a-fA-F\d]{4,}\b',log2) or log2=='<\d>' or log2=='-<\d>'):
            return 1, None
        # if((re.search(r'\d',log1) or '<\d>' in log1) and (re.search(r'\d',log2) or '<\d>' in log2)):
        #     return 0.5, None
        return 0,None
    ...
    for delimiter_now in delimiters_same_pattern:
        ...
        for i in range(len(split_pos1)-1):
            ...
            for key in brackets2:
                ...
                if len(list_new) > 0:
                    sub_bracket2[key] = list_new
            sim_tmp=(sim_log2log(log1_new,sub_delimiters1,sub_bracket1,log2_new,sub_delimiters2,sub_bracket2))[0]
            sim+=sim_tmp/(2+len(pos1[delimiter_now]))
            if (len(split_pos1) == 2):
                sim=sim_tmp
        if(max_sim<sim):
            max_sim=sim
...
def sim_log_node(node,log,delimiters,brackets):
    ...
    if (node.isLeaf or (not delimiters and not brackets)):
        if(node.is16):
            if re.match(r'\b(\-?\+?\d+)\b|\b0[Xx][a-fA-F\d]+\b|\b[a-fA-F\d]{4,}\b', log) or log=='<\d>' or log=='-<\d>':
                return 1
        # if(node.hasDigit):
        #     if(re.search(r'\d',log) or '<\d>' in log):
        #         return 0.5
        return 0

    if(',' in delimiters.keys()):
        logs=log.split(',')
        if("" not in logs and len(logs)>1):
            fold=True
            if(re.search(r'\d', logs[0]) or'<\d>' in logs[0]):
                for i in range(len(logs)-1):
                    if(re.search(r'\d', logs[i+1]) or'<\d>' in logs[i+1]):
                        if(not can_fold(logs[i],logs[i+1],{},{},{},{},True,True)):
                            fold = False
                            break
                    else:
                        fold=False
        ...
        for i in range(len(children)):
            ...
            for key in delimiters.keys():
                ...
                for key in brackets:
                    ...
                    if len(list_new) > 0:
                        sub_bracket[key] = list_new
            sim_tmp=(sim_log_node(child,sub_log,sub_delimiters,sub_bracket))
            sim+=sim_tmp/(len(children)+1)
            if(len(children)==1):
                sim=sim_tmp
        return sim
    else:
...
def fold_nodes(trivial_log, Split_words_bySpace, delimiters, brackets, hasDigit):
    ...
    while (id < len(Split_words_bySpace) - 1):
        if (
                can_fold(Split_words_bySpace[id], Split_words_bySpace[id + 1], delimiters[id], delimiters[id + 1],
                         brackets[id],
                         brackets[id + 1], hasDigit[id], hasDigit[id + 1])):
